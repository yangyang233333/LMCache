...
AAA_PATH="${SCRIPT_DIR#"${REPO_ROOT}/"}"

if [[ "${SCRIPT_DIR}" == "${REPO_ROOT}" || "${AAA_PATH}" == "${SCRIPT_DIR}" ]]; then
  echo "AAA directory is not inside the Git repository" >&2
  exit 1
fi
...
# Stage only AAA. Changes elsewhere in the repository remain untouched.
git add -A -- "${AAA_PATH}"

if ! git diff --cached --quiet -- "${AAA_PATH}"; then
  git commit --only --author="${COMMIT_AUTHOR}" -m "${COMMIT_MESSAGE}" -- "${AAA_PATH}"
else
  echo "No changes under ${AAA_PATH} to commit."
fi
